...
public final class SftpStorageClient implements StorageClient, BucketOperations {

    private final String name;
    private final String root;
    private final SftpConnectionFactory connections;

    SftpStorageClient(String name, String root, SftpConnectionFactory connections) {
    ...
    public String providerType() {
        return SftpStorageProvider.TYPE;
    }
    ...
    public ObjectMetadata stat(StorageObject object) {
        Objects.requireNonNull(object, "object");
        try {
            SftpConnectionFactory.Entry entry = connections.execute(
        ...
        try {
            return connections.execute(session -> {
                ...
                return session.list(root).stream()
                        .filter(SftpConnectionFactory.Entry::directory)
    ...
    public void close() {
        try {
            ...
            throw map("Close SFTP client", new IOException(exception));
        }
    }

    private void scan(
            SftpConnectionFactory.Session session,
            String bucket,
            String directory,
            List<ObjectMetadata> result) throws IOException {
        for (SftpConnectionFactory.Entry entry : session.list(directory)) {
            if (entry.directory()) {
    ...
    private static ObjectMetadata metadata(StorageObject object, SftpConnectionFactory.Entry entry) {
    ...
    private static String validateBucket(String bucket) {
        if (bucket == null || bucket.isBlank() || bucket.contains("/") || bucket.contains("\\")
            ...
            throw invalid("SFTP bucket must be a single safe directory name");
        }
        return bucket;
    }
    ...
    private static String validateObjectKey(String key) {
        if (key == null || key.isBlank() || key.startsWith("/") || key.endsWith("/")
            ...
            throw invalid("SFTP object key must be a safe relative file path");
        }
        for (String segment : key.split("/", -1)) {
            if (segment.isEmpty() || ".".equals(segment) || "..".equals(segment)) {
                throw invalid("SFTP object key contains an invalid path segment");
            }
        }
        return key;
    }
    ...
    private StorageException map(String operation, IOException exception) {
        ...
        if (exception instanceof JschSftpConnectionFactory.SftpAuthenticationException
                || exception instanceof JschSftpConnectionFactory.SftpAuthorizationException) {
            return new StorageException(
                    StorageErrorCode.UNAUTHORIZED, operation + " was rejected by SFTP",
                    providerType(), exception, false);
        }
        if (exception instanceof JschSftpConnectionFactory.SftpConflictException) {
            return new StorageException(
                    StorageErrorCode.CONFLICT, operation + " conflicts with remote SFTP state",
                    providerType(), exception, false);
        }
        return new StorageException(
    ...
    private static void safeDelete(
            SftpConnectionFactory.Session session, String path, IOException original) {
        try {
            if (session.fileExists(path)) {
                session.deleteFile(path);
            }
    ...
    private static StorageException invalid(String message) {
        return new StorageException(
                StorageErrorCode.INVALID_REQUEST, message, SftpStorageProvider.TYPE, null, false);
    }
}
