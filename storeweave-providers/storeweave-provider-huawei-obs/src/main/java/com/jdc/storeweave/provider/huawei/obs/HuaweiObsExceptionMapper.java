...
final class HuaweiObsExceptionMapper {
    ...
    private HuaweiObsExceptionMapper() {
    }

    static StorageException map(String operation, Throwable exception) {
        if (exception instanceof StorageException storageException) {
            return storageException;
        }
        if (exception instanceof ObsException obsException) {
            String code = obsException.getErrorCode();
            int status = obsException.getResponseCode();
            boolean retryable = status == 408 || status == 429 || status >= 500
                    || (code != null && RETRYABLE.contains(code));
            return new StorageException(
                    errorCode(code, status),
                    operation + " failed with Huawei OBS" + codeSuffix(code),
                    HuaweiObsStorageProvider.TYPE,
                    obsException,
                    retryable);
        }
        if (exception instanceof IOException) {
            return new StorageException(
                    ...
                    operation + " failed because Huawei OBS could not be reached",
                    HuaweiObsStorageProvider.TYPE,
                    exception,
                    true);
        }
    ...
    static boolean isNotFound(RuntimeException exception) {
        return exception instanceof ObsException obsException
                && (obsException.getResponseCode() == 404
                || (obsException.getErrorCode() != null
                && NOT_FOUND.contains(obsException.getErrorCode())));
    }

    static boolean isMissingLifecycle(RuntimeException exception) {
        return exception instanceof ObsException obsException
                && (obsException.getResponseCode() == 404
                || "NoSuchLifecycleConfiguration".equals(obsException.getErrorCode()));
    }
